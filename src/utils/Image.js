...
import { image, inside, transparent, inverted, element, spinner, loading, loaded, cover, contain } from './Image.module.css'
...
export const Image = ({ src, alt, height, width, className, proportionalHeight, transparent:isTransparent, invert:isInverted, style, loading:isLoading, loaded:isLoaded, failed:isFailed, cover:isCover, contain:isContain=true, imageRef }) => (
  <span ref={imageRef} title={alt} className={classnames((isTransparent ? transparent : (isInverted ? inverted : image)), className,'Image')} style={style}>

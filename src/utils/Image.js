...
export const Image = ({ url, src, alt, height, width, className, proportionalHeight, transparent:isTransparent, invert:isInverted, style, loading:isLoading, loaded:isLoaded, failed:isFailed, cover:isCover, contain:isContain=true, imageRef, children }) => (
  <span ref={imageRef} title={alt} className={classnames((isTransparent ? transparent : (isInverted ? inverted : (isFailed?failed:image))), className,'Image')} style={style}>
    <span className={classnames(isLoaded ? loaded : inside, (isCover ? cover : isContain !== false && contain ))} style={backgroundImage(src||url)}/>
